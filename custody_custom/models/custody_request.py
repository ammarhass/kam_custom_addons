...
class CustodyRequest(models.Model):
    ...
    is_third_approval = fields.Boolean(
        compute='_compute_third_approval',
        store=False
    )
    is_fourth_approval = fields.Boolean(
        compute='_compute_fourth_approval',
        store=False
    )
    ...
    third_approval = fields.Many2one(related='employee_id.third_approval')
    fourth_approval = fields.Many2one(related='employee_id.fourth_approval')
    company_id = fields.Many2one(
        comodel_name="res.company", string="Company", index=1, default=lambda self: self.env.company
    )

    state = fields.Selection(string="", selection=[('draft', 'Draft'), ('first_approve', 'First Approve'),
                                                   ('second_approve', 'Second Approve'),
                                                   ('third_approve', 'Third Approve'), ('approved', 'Approved'),
                                                   ('paid', 'Paid'),
    ...
    def _compute_third_approval(self):
        """Compute if the current user is the second approver."""
        for rec in self:
            rec.is_third_approval = (
                    rec.employee_id
                    and rec.employee_id.third_approval
                    and rec.employee_id.third_approval.id == self.env.user.employee_id.id
            )

    def _compute_fourth_approval(self):
        """Compute if the current user is the second approver."""
        for rec in self:
            rec.is_fourth_approval = (
                    rec.employee_id
                    and rec.employee_id.fourth_approval
                    and rec.employee_id.fourth_approval.id == self.env.user.employee_id.id
            )
    ...
    def action_second_approve(self):
        ...
        for rec in self:
            ...
            if rec.requested_value <= rec.employee_id.second_amount_to:
                rec.set_approved()
            else:
                mail_template = self.env.ref('custody_custom.send_second_approve_email')
                mail_template.send_mail(rec.id, force_send=True, email_values={'res_id': rec.id})
    ...
    def action_third_approve(self):
        self = self.sudo()
        for rec in self:
            rec.write({'state': 'third_approve'})
            if rec.requested_value <= rec.employee_id.third_amount_to:
                rec.set_approved()
            else:
                mail_template = self.env.ref('custody_custom.send_second_approve_email')
                mail_template.send_mail(rec.id, force_send=True, email_values={'res_id': rec.id})
